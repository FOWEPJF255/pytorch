...
class AOTInductorTestsTemplate:
    ...
    def test_kernel_profile_scope_hoists_per_graph_workspace(self):
        ...
        example_inputs = (
            torch.randn(4, 524288, device=self.device),
            torch.randn(6, 262144, device=self.device),
        )
        ...
        with config.patch(
            ...
            first_block = re.search(r"\{\s*KernelContextGuard", code)
            self.assertIsNotNone(first_block, "no profiling block was emitted")
            # Both kernels here are cooperative reductions, so the first block
            # is the one that asks for the semaphores; the declaration has to
            # land in front of it to stay visible to the second.
            self.assertLess(semaphores.start(), first_block.start())
