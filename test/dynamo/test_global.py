...
class TestDeleteGlobal(torch._dynamo.test_case.TestCase):
    ...
    def test_delete_missing_global_caught_presence_flip(self):
        ...
        # branch and never deletes. Guarding it needs a Source that resolves to
        # the f_globals *dict*: GlobalSource resolves by reading the name, so it
        # cannot express "absent" (a DICT_NOT_CONTAINS guard on it fails with
        # KeyError from GlobalSource.get_value). See the LOAD_GLOBAL control
        # below, which has the identical hole.
        global _dg_a
        with temp_globals(globals(), _dg_a=_ABSENT):
